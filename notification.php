...
$notification = $conn->query("SELECT u.name, n.title, n.content, n.date_updated, n.task_id FROM notification_task n join users u on u.id = n.user_id join tasks t on t.id = n.task_id where n.user_type = $login_type order by n.date_updated desc");
$notif = $conn->query("SELECT * FROM notification_task WHERE user_type=".$_SESSION['login_type']);
		...
		<div class="row">
			<div class="card col-md-12">
				<div class="card-body">
					...
					?>
						<tr class='file-item' file-id="<?php echo $row['task_id'] ?>" data-id="<?php echo $row['task_id'] ?>" data-name="<?php echo $name ?>">
							<td><i class="to_file"><?php echo $row['name'] ?></i></td>
							<td><i class="to_file"><?php echo $row['title'] ?></i></td>
							<td><i class="to_file"><?php echo $row['content'] ?></i></td>
